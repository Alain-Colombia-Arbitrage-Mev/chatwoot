...
RSpec.describe Account, type: :model do
  ...
  describe 'default features' do
    ...
    it 'enables Captain V2 for new self-hosted enterprise accounts' do
      ...
      expect(account).to be_feature_enabled('captain_tasks')
      expect(account).to be_feature_enabled('custom_tools')
      expect(account.captain_preferences[:models]['assistant']).to eq(
        Llm::FeatureRouter::CAPTAIN_V2_ASSISTANT_MODEL
      )
