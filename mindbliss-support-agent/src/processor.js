...
export class WebhookProcessor {
  ...
  async process(payload, deliveryId = '') {
    ...
    if (
      await this.chatwoot
        .hasNoteMarker(accountId, conversationId, marker)
        .catch(() => false)
    ) {
    ...
    const supportResult = await this.supportBrain.ask(
      prompt,
    ...
    await this.chatwoot
      ...
      .catch(() => null);
    await this.chatwoot
      .setPriority(accountId, conversationId, triage.priority)
      .catch(() => null);
  ...
  async processResolvedConversation(payload, deliveryId = '') {
    if (this.config.resolutionMemory?.enabled === false)
      return { status: 'ignored', reason: 'resolution_memory_disabled' };

    const accountId = getWebhookAccountId(payload);
    const conversationId = getWebhookConversationId(payload);
    if (!accountId || !conversationId) {
      return { status: 'ignored', reason: 'missing_chatwoot_context' };
    }

    const maxMessages = this.config.resolutionMemory?.maxMessages || 150;
    ...
    const chunks = buildResolutionMemoryChunks({
      ...
      opts: {
        includePrivate: this.config.resolutionMemory?.includePrivate === true,
        chunkMaxChars: this.config.resolutionMemory?.chunkMaxChars || 3500,
      },
    });
    if (chunks.length === 0)
      return { status: 'ignored', reason: 'empty_resolution' };

    let stored = 0;
    for (const chunk of chunks) {
      const triage = normalizeSupportTriage(classifySupportText(chunk.content));
      const ok = await this.memory
        .store(chunk.payload, triage, {
    ...
    if (stored > 0) {
      await this.chatwoot
        .addLabels(accountId, conversationId, [
          `${this.config.chatwoot.labelPrefix}_memory_trained`,
        ])
        .catch(() => null);
    }
    ...
    return {
      status:
        stored > 0 ? 'resolution_memory_stored' : 'resolution_memory_failed',
  ...
  async processKnowledgeCommand(payload, deliveryId = '') {
    if (this.config.knowledge?.enabled === false)
      return { status: 'ignored', reason: 'knowledge_commands_disabled' };
    ...
    if (command.valid) {
      ...
      const memoryPayload = buildKnowledgePayload(
        payload,
        command,
        idempotencyKey
      );
      ...
      await this.chatwoot
        .addLabels(
          accountId,
          conversationId,
          this.knowledgeLabels(command, stored)
        )
        .catch(() => null);
    }
