...
test('allows local qdrant without api key when network is private', () => {
  ...
  assert.equal(cfg.resolutionMemory.enabled, true);
  assert.equal(cfg.resolutionMemory.includePrivate, false);
  assert.equal(cfg.resolutionMemory.maxMessages, 150);
...
test('configures OpenRouter support brain without legacy VP service', () => {
  ...
  assert.equal(
    cfg.support.openRouter.chatUrl,
    'https://openrouter.ai/api/v1/chat/completions'
  );
  assert.equal(cfg.support.openRouter.model, 'upstage/solar-pro4');
  assert.equal(
    cfg.support.openRouter.referer,
    'https://soporte.mindblisspower.com'
  );
